...
label = tk.Label(root, text='\nВнесите задачу в План', background='aquamarine1')
label.grid(row=0, column=0, columnspan=2, pady=10)

entry = tk.Entry(root, justify='center', bg='aquamarine1')
entry.grid(row=1, column=0, columnspan=2, pady=10, sticky='ew')
entry.bind('<KeyRelease>', on_entry_change)

button = tk.Button(root, text='Нажать для записи задачи в План', command=task_writing, bg='blue violet', fg='white')
button.grid(row=2, column=0, columnspan=2, pady=10)

delete_button = tk.Button(root, text='Удалить задачу из Плана', command=task_deleting, bg='blue violet', fg='white')
delete_button.grid(row=3, column=0, columnspan=2, pady=10)

mark_button = tk.Button(root, text='Отметить задачу как выполненную', command=mark_task, bg='blue violet', fg='white')
mark_button.grid(row=4, column=0, columnspan=2, pady=10)

text1 = tk.Label(root, text='Список задач в Плане', justify='left', bg='aquamarine1')
text1.grid(row=5, column=0, pady=10)

text2 = tk.Label(root, text='Выполненные задачи', justify='left', bg='aquamarine1')
text2.grid(row=5, column=1, pady=10)

task_listBox = tk.Listbox(root, height=34, width=40, bg='cornsilk3', fg='blue violet')
task_listBox.grid(row=6, column=0, pady=10, sticky='ew')

completed_listBox = tk.Listbox(root, height=34, width=40, bg='cornsilk3', fg='blue violet')
